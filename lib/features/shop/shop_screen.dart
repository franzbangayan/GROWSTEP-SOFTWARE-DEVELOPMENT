...
void _refreshUser() {
  ...
  Widget _buildAvatarPreview() {
    ...
    // Find equipped item (for name display)
 ...
 Widget _buildCategoryTabs() {
  ...
  return Padding(
    ...
    child: Row(
      children: [
        ...
        // Coin balance — synced with home style
        Container(
          ...
          child: Row(
            children: [
              const Icon(Icons.monetization_on_rounded, color: _coinYellow, size: 14),
              const SizedBox(width: 4),
              Text(
                '${_user?.coins ?? 0}',
                style: const TextStyle(
              ...
              GestureDetector(
                onTap: () async {
                  if (_user == null) return;
                  final updated = _user!.copyWith(coins: _user!.coins + 200);
                  await AuthService.saveUser(updated);
                  setState(() => _user = updated);
                },
                child: Container(
  ...
  Widget _buildItemGrid() {
    final allItems = ShopItemModel.byCategory(_activeCategory);
    
    // Filter items to only show those that match the current avatar's theme
    final items = allItems.where((item) {
      final avatarId = _user?.avatarId ?? 'oakley';
      
      // If it's a default look, only show the one for our character
      if (item.id.startsWith('normal_')) {
        return item.id == 'normal_$avatarId';
      }
      
      // Match the prefix (e.g. 'glowie_' for 'glowie')
      return item.id.startsWith(avatarId);
    }).toList();
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.shopping_bag_outlined, 
                size: 64, color: _textGrey.withOpacity(0.3)),
            const SizedBox(height: 16),
            Text(
              'More items coming soon for this character!',
